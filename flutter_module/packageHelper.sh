...
if [ -f "$PROPERTIES" ]; then
  ...
  if [[ -z "$mavenRepositoryV" ]] && [[ -z "$mavenSnapshotsV" ]]; then
...
if [ $isRelease = "true" ]; then
  ...
  echo "当前打包Debug版本，版本号:$buildVersion,running..."
fi
...
if [ $isRelease = "true" ]; then
  ...
  showBuildVersion="$buildVersionV-SNAPSHOT"
...
function renameAllPomArtifactId() {
  find build/host/outputs/repo -name "*.pom" | while read -r file; do
    sed -i -e "s/>$1</>$2</g" $file
  done
}

#第一次遍历repo文件夹下所有后缀名为.aar的文件
find build/host/outputs/repo -name "*.aar" | while read -r file; do
  #当前aar文件所在的目录
